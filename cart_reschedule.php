...
$("#datError").html('Loading.....'); 
  $.ajax( {
					...
					//	if(html.tostring()=='deleted File'){
						var urls = "cart_reschedule.php?product="+getData+"&transid="+tempTransId+"&stat="+stats;
//alert(urls);
...
$("#datError").html('Loading.....'); 
  $.ajax( {
						...
						$("#datError").html(html).show();
					//	if(html.tostring()=='deleted File'){
						var urls = "cart_reschedule.php?product="+getDatas+"&transid="+tempTransId+"&stat="+stats;
//alert(urls);
 $(window.location).attr('href', urls);
						//}
					}
...
<?php if($stat=='Detox'){?>
  ...
  <?php $i=1;$csimg="";
				...
				$csimg="subscription-box white group";
				}else{
				
				$csimg="subscription-box gray group";
				}							
			
					?>
	 <section class="<?=$csimg?>" >		
	<div class="wrapper group">
    	<?php /*?><div class="quantity"><img src="images/icon-basket-detox-blue.png"  height="60px" alt="Detox" /><?php echo substr($row_ct['product_type'],6); ?></div><?php */?>
  ...
  <?php  $i=1;$csimg="";
	...
	<div class="wrapper group">
        ...
        <!--    <span class="last group">3</span>  -->
         ...
         <div class="quantity"><input name="trans_tmp_id"  id="trans_tmp_id<?=$tid?>" value="<?=$tmp_id?>" type="hidden">
        <?php  echo '1';    ?>        <!--    <span class="last group">3</span>  -->
        </div>
		<div class="quantity">
        	<?php  echo $row_ct['delivery_date'];  ?>
...
$sql_ct = "select * from farmhouse_temp_cart_fh  where customer_id = '$cid' and product_type='$select_type'";
				...
				$csimg="retail-box white group";
				}else{
				
				$csimg="retail-box gray group";
				}	
				?>
				 <section class="<?=$csimg?>" >		
	    <div class="wrapper">
...
<section class="gift-card-check-out border-top group">
    ...
    <!--	<div class="gift-card f-right"><input type="checkbox" name="gift-card" /> Use gift card <input type="text" name="gift-text" placeholder="Fill the Serial Number" /><input type="submit" name="submit" value="Enter" /></div>-->
        ...
        <div class="checkout"><a href="card1.php">Check Out</a></div>
