...
<section class="back-button fixed-top group">
	<div class="wrapper group">
    	<a href="add_to_cart.php?back_id_lf=<?php echo $_GET['id'];?>">BACK</a>
    </div>
</section>

<section class="main-title group">
	<div class="wrapper group">
    	<h2>SCHEDULE MY LIFESTYLE</h2>
    </div>
</section>
	...
	<div class="wrapper group">
        ...
        <aside class="list group">
        	...
        	<figure class="group">
                ...
                <div class="type">
				<?php $sql_dtx = "select lifestyle_name from farmhouse_temp_schedule_lifestyle where lifestyle_id = '$dtx_id_detox' ";
				$res_dtx = mysqli_query($conn,$sql_dtx); 
				$row_dtx = mysqli_fetch_array($res_dtx);?>
                	<select class="SlectBox" name="type"><?php  $getname = trim($row_ls['lifestyle_name'],"lifestyle-");?>
...
?>
	...
	//	for($j = 1; $j<=8; $j++) {   ?>
		...
		<!--loop start for no of people-->
			 ...
			 <div class="row group">
            	<div class="col-1"><input type="radio" name="iCheck1" id="iCheck1" value="A"><span>A</span></div>
				<?php 
				for($i = 2; $i<=8; $i++) { ?>
...
return false;
	...
	var x = chkvalue;
	if(x=="A" && z!="") {
	window.location.href = "ls_schedule2.php?id=<?=$dtx_id_detox?>&per_name="+x+"&date="+z+"&tid="+t_id+"&ppl=<?=$no_ppl?>&ls=<?=$no_dtx?>";
	} //ends if2
	if(x=="B" && z!="") {
	window.location.href = "ls_schedule2.php?id=<?=$dtx_id_detox?>&per_name="+x+"&date="+z+"&tid="+t_id+"&ppl=<?=$no_ppl?>&ls=<?=$no_dtx?>";
	} //ends if2
