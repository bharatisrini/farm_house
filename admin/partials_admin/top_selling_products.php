...
if(isset($_POST['submit'])) {
	...
	echo "<h2 align=center>Top Selling Items From <span style='color:#F90'>$date1</span> to <span style='color:#F90'>$date2</span>  </h2><table align='center' width='80%' border='0' cellpadding=5 cellspacing=2>
	
	<tr height='35' bgcolor='#00aeef'>
    
    <th align='center'>Product Name</th>
	<th align = 'center'>Image</th>
    <th align = 'center'>Total items sold</th>
     </tr>";
	 
	 $sql_showorders = "SELECT name, (SELECT count(product_name) FROM farmhouse_program_items a where b.name=a.product_name ) as items,
(select distinct count(no_people) from farmhouse_delivery_schedule c, farmhouse_program_items d where d.program_name=c.product_type and d.product_name=b.name and order_id!=0 and delivery_date between '$date1' and '$date2') as noofppl,
(select distinct  count(no_people) from farmhouse_delivery_schedule e where e.product_type=b.name and order_id!=0 and delivery_date between '$date1' and '$date2' and program_type='Retail') as retails
FROM farmhouse_juice_details b where countrylang_id='1' order by items desc, noofppl desc";
	...
	echo "<td>".$row_showorders['items']*$row_showorders['noofppl']."</td>"; }
	else {
	echo "<td>".$row_showorders['items']*$row_showorders['noofppl']*$row_showorders['retails']."</td>"; }
