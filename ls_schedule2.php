...
//alert(dataPass);
url="tempDeliveryMorelifeData.php";
...
<section class="back-button fixed-top group">
	<div class="wrapper group">
    	<a href="add_to_cart.php?back_id3=<?=$transId?>&id=<?=$dtx_id_detox?>&ppl=<?=$no_ppl?>&ls=<?=$no_dtx?>">BACK</a>
    </div>
</section>
...
<section class="plan">
	<div class="wrapper">
    	<aside class="pick-plan group">Schedule:  <?=$_REQUEST["date"]?>, <?=date("D", strtotime($_REQUEST["date"]))?></aside>
        <div class="clear"></div>
...
$m=$n-1;
	...
	<div class="row group" ondrop="drop(event)" ondragover="allowDrop(event)">
				...
				if($select_box==''){ if($i==2) { $classimg="col-".$i." can-chedule"; $imgDefault="<img src='address_icons/1.png' id='df".(($i+$m)-1)."'>"; $drpdrop="ondrop='drop(event,".(($i+$m)-1).")' ondragover='allowDrop(event)'"; } else { $classimg="col-".$i; $imgDefault="";} }elseif($select_box=='A'){  if($i==2 || $i==5 || $i==7) { $classimg="col-".$i." can-chedule"; $imgDefault="<img src='address_icons/1.png' id='df".(($i+$m)-1)."'>"; $drpdrop="ondrop='drop(event,".(($i+$m)-1).")' ondragover='allowDrop(event)'";  } else { $classimg="col-".$i; $imgDefault=""; $drpdrop="";} }elseif($select_box=='B' ){
				if($i==2 || $i==4 || $i==6) { $classimg="col-".$i." can-chedule"; $imgDefault="<img src='address_icons/1.png' id='df".(($i+$m)-1)."'>";$drpdrop="ondrop='drop(event,".(($i+$m)-1).")' ondragover='allowDrop(event)'"; } else { $classimg="col-".$i; $imgDefault="";$drpdrop=""; }
				}elseif($select_box=='C'){ if($i==2 || $i==6 || $i==8) { $classimg="col-".$i." can-chedule"; $imgDefault="<img src='address_icons/1.png' id='df".(($i+$m)-1)."'>";$drpdrop="ondrop='drop(event,".(($i+$m)-1).")' ondragover='allowDrop(event)'"; } else { $classimg="col-".$i; $imgDefault=""; $drpdrop="";}
				}
				?>
				
                <div class="<?=$classimg?>" id="c<?=($i+$m)-1?>" style="color: #000000; font-size:xx-medium;" ondrop="drop(event, 'saddr<?=($i+$m)-1?>')" ondragover="allowDrop(event)"><?=$imgDefault?><input name="sdate[]" id="sdate<?=($i+$m)-1?>" type="hidden" size="8"> <input name="saddr[]" id="saddr<?=($i+$m)-1?>" value='<?=$custAddressId?>' type="hidden" size="8"></div>
				<?php } ?>
			</div>
			<?php } ?>
        ...
        <div class="address-list group">
        	...
        	<figure class="group">
				...
				<img src="<?php echo $address_icon; ?>" id="drag<?=$row_addr['address_id']?>" draggable="true" ondragstart="drag(event,<?=$row_addr['address_id']?>)"/><!--</a>--></div>
	...
	<div class="wrapper group">
		...
		<input type="hidden" name="shift" id="shift" value='<?=$sh_type?>'>
   	  <a href="card1.php" class="add-card" id="submitData">Add Into</a>    </div>
</section>
