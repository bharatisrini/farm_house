  ...
  <?php 
	...
	<div class="wrapper group">
        ...
        <div class="manage">
        	...
        	<span class="last group"><?php if($row_ct['no_days_detox']>=4){ ?><a href="<?php if(isset($cid)) { ?>cart_reschedule.php?product=<?=$row_ct['detox_name']?>&transid=<?=$row_ct['trans_id']?>&stat=Detox<?php } else echo "#"; ?>" id="<?=$row_ct['detox_name']?>" class="reschedule" >Reschedule</a><?php }?><a href="#" id="Detox" name="<?=$row_ct['detox_name']?>" class="delete">Delete</a></span>
...
<section class="subscription-box border-bottom group">
 <?php 
					...
					?>
	<div class="wrapper group">
    	<div class="check"><input type="checkbox" name="Subscriptions-detox" class="chslc" checked /><input type="hidden" name="transId" id="transId" value="<?=$row_ct_ls['trans_id']?>"/></div>
        <div class="image" style="background-image: url(images/icon-basket-detox-blue-blank.png); height:72px; width:86px; color:#FFFFFF; font-weight:800; vertical-align: middle;  color:#FFFFFF; font-size:15px; font-weight:800; text-align:center; background-repeat: no-repeat; ">
        ...
        <div class="manage">
        	<span class="last group"><?php if($row_ct_ls['no_days_beauty']>=4){ ?><a href="<?php if(isset($cid)) { ?>cart_reschedule.php?product=<?=$row_ct_ls['lifestyle_name']?>&transid=<?=$row_ct_ls['trans_id']?>&stat=Lifestyle<?php } else echo "#"; ?>" class="reschedule">Reschedule</a><?php }?><a href="#" id="Beauty" name="<?=$lsname?>" class="delete">Delete</a></span>
        </div>
    </div>
	<?php }?>
</section>
