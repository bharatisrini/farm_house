...
if(isset($_POST['submit'])) {
	...
	echo "<br><br><h4 align=center>$date1</h4><table align='center' width='80%' border='0' cellpadding=5 cellspacing=2>
	
	<tr height='35' bgcolor='#00aeef'>
    
    <th>Product Name</th>
	<th>Image</th>
    <th>Total items to be prepared</th>
     </tr>";
	 ...
	 //SELECT detox_name, (select distinct count( no_people) from  farmhouse_delivery_schedule b where a.detox_name=b.product_type and order_id!=0 and delivery_date between '2015-03-20' and '2015-03-24') as nop FROM farmhouse_detox_program a  ;
	 
	 $sql_showorders = "SELECT name, (SELECT count(product_name) FROM farmhouse_program_items a where b.name=a.product_name ) as items,
(select distinct count(no_people) from farmhouse_delivery_schedule c, farmhouse_program_items d where d.program_name=c.product_type and d.product_name=b.name and order_id!=0 and delivery_date='$date1' and delivery_status=1 ) as noofppl,
(select distinct  count(no_people) from farmhouse_delivery_schedule e where e.product_type=b.name and order_id!=0 and delivery_date='$date1' and delivery_status=1 and program_type='Retail') as retails
FROM farmhouse_juice_details b where countrylang_id='1'";
